...
export function AboutStory() {
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <BentoCard title="My Story" icon={<BookOpen size={16} />} eyebrow="PERSONAL" className="min-h-[260px] lg:col-span-2">
        <div className="space-y-3">{personalStory.paragraphs.map((p, i) => <p key={i}>{p}</p>)}</div>
      </BentoCard>
      <div className="flex flex-col gap-6">
        <BentoCard title="Career Objectives" icon={<Target size={16} />} eyebrow="GOALS">
          <ul className="space-y-2">{careerObjectives.map((goal, i) => <li key={i} className="flex gap-2"><span aria-hidden className="text-signal">→</span><span>{goal}</span></li>)}</ul>
        </BentoCard>
